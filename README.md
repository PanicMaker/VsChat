# WeChat ClawBot — VSCode Extension
...
│   ├── clawbot-client.ts         # 核心客户端：登录、轮询、收发消息、CDN 上传
...
| **客户端** | `clawbot-client.ts` | QR 码登录、长轮询消息、文本/图片发送、AES 加解密、CDN 文件上传 |
...
1. **登录流程**: `extension.ts` → `ClawBotClient.login()` → 获取 QR 码 → Webview 展示 → 用户扫码 → 获取 `bot_token` → 存入 SecretStorage + 文件
2. **接收消息**: `ClawBotClient.pollLoop()` → `getupdates` API → `processMessages()` → 解密图片 → 存入 DB → 触发 `onMessage` 事件 → `ChatViewProvider` → Webview `renderMessage()`
3. **发送文本**: Webview `sendMessage` → `ChatViewProvider.handleWebViewMessage()` → `ClawBotClient.sendText()` → `sendmessage` API
4. **发送图片**: Webview 选择文件 → base64 传递 → 写入临时文件 → `ClawBotClient.sendImage()` → AES 加密 → CDN 上传 → `sendmessage` API
...
1. 安装扩展后，VSCode 活动栏出现 💬 **WeChat ClawBot** 图标
2. 打开聊天面板，执行命令 `ClawBot: Login`
...
| 命令 | 说明 |
|------|------|
| `ClawBot: Login` | 扫码登录微信 |
| `ClawBot: Clear Chat History` | 清除所有聊天记录 |
| `ClawBot: Disconnect` | 断开连接并登出 |

### 配置项

| 配置 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `clawbot.proxyUrl` | string | `""` | HTTP 代理 URL，留空使用系统代理 |
| `clawbot.fromUserId` | string | `""` | 指定聊天对象 ID，留空自动检测 |
